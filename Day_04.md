...
A naive forecast works as a simple baseline to compare against more advanced models. The idea is straightforward: the next period’s value is just the last observed value.

I started by calculating distance_per_dollar for each month. To do that, I extracted the year and month from request_date using strftime and then summed the distance and cost for each month.

In the second CTE, I used the LAG window function to pull the previous month’s value and treat it as the forecast for the current month. In the final step, I calculated the RMSE in the main query.
...
WITH current_month AS(
    SELECT  request_id, strftime('%Y-%m', request_date) as month, 
...
forecast AS(
    SELECT *,
           LAG(distance_per_dollar, 1) 
                OVER (ORDER BY month) AS forecasted_value
    FROM current_month
)
SELECT SQRT(AVG(POW((distance_per_dollar - forecasted_value) * (distance_per_dollar - forecasted_value), 2)))AS RMSE
 FROM  forecast
 WHERE orecasted_value IS NOT NULL;
